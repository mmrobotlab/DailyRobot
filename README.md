...
本文介绍了一种名为EmbodiedGPT的多模态基础模型，用于实现具有多模态理解和执行能力的具身化智能体。为了实现这一目标，作者进行了以下努力：
(i) 构建了一个大规模的具身化规划数据集EgoCOT，该数据集由精心挑选的Ego4D数据集中的视频和相应的高质量语言指令组成。具体而言，作者采用了“思维链”模式生成了一系列子目标，以实现有效的具身化规划。
(ii) 引入了一种高质量计划生成的高效训练方法，通过将7B大型语言模型（LLM）适应EgoCOT数据集进行前缀调整，从而使EmbodiedGPT具备高质量的计划生成能力。
(iii) 引入了一种从LLM生成的规划查询中提取任务相关特征的范式，以形成高层规划和低层控制之间的闭环。通过广泛的实验，证明了EmbodiedGPT在具身化任务（包括具身化规划、具身化控制、视觉字幕生成和视觉问答）上的有效性。值得注意的是，与使用Ego4D数据集微调的BLIP-2基线相比，EmbodiedGPT在Franka Kitchen基准测试中的具身化控制任务成功率显著提高了1.6倍，在Meta-World基准测试中提高了1.3倍。

- 评估图像输入任务：
为了评估不同端到端模型在生成字幕和规划方面的质量，作者进行了一项用户研究，共有30名参与者。该研究包括来自MS-COCO数据集的10个图像字幕任务案例，不同具身化智能体模拟器中的5个具身化规划场景，以及伴随的5个真实场景的具身化规划任务。参与者被要求使用从1到10的评分系统，在五个维度上对不同模型生成的字幕进行评分：物体识别准确性、空间关系理解程度、答案中的冗余程度、规划的合理性和可执行性。表1显示了所有参与者对不同模型的平均评分。结果表明，尽管EmbodiedGPT在语言模型中只有7B个参数，但在物体识别和空间关系理解方面，其达到了与LLaVA-13B模型相当的水平。此外，EmbodiedGPT在与给定具身化智能体任务相关的内容中生成的冗余内容较少，并且产生了最合理和可执行的规划输出。作者还将EmbodiedGPT的性能与Visual ChatGPT进行了比较，Visual ChatGPT采用分层方法，通过组合多个预训练的视觉模型和语言模型来回答问题。在Virtual-Home基准测试中，Visual ChatGPT使用视觉字幕模型生成密集字幕，然后将其传递给ChatGPT以得出解决方案。如图3所示，由于仅依赖字幕模型提取视觉信息的局限性，Visual ChatGPT无法找到衣架，导致性能较差。

- 具身化规划任务：
在具身化规划任务中，参与者被要求根据给定的图像进行具身化规划。EmbodiedGPT生成的规划结果如下：
1. 图像是一个带有门口的房间，门上方有一个墙上的时钟和一幅挂在墙上的图片。
2. 场景中有一个衣架，位于左侧。
3. 衣架靠近门口的右侧，位于女人的图片的左侧可见。
4. 衣架靠近图像的边缘，放置方式不会遮挡时钟或女人的图片。

- 具身化控制任务：
在具身化控制任务中，参与者被要求编写机器人执行任务的详细计划。EmbodiedGPT生成的计划如下：
步骤1：识别木箱和钉子。
步骤2：
